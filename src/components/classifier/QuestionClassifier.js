...
import {AnswerButton, GuideButton, SubmitButton} from './ClassifierButton';
...
class QuestionClassifier extends Component {
    ...
    finishTutorial() {
        if (this.props.needsTutorial) {
            this.props.classifierActions.setTutorialCompleted(
                this.props.route.params.workflow.id,
    ...
    submitClassification() {
        return () => {
            const {classifierActions, workflow, subject} = this.props;
            const {id, first_task} = workflow;
            const {answerSelected, imageDimensions} = this.state;

            this.scrollView.scrollTo({x: 0, y: 0});
            classifierActions.addAnnotationToTask(
                id,
                first_task,
                answerSelected,
                false,
            );
            classifierActions.saveClassification(
                workflow,
                subject,
                imageDimensions,
            );
    ...
    render() {
        ...
        const {project, workflow, isBetaMode} = this.props.route.params;
        ...
        const renderTutorial = () => (
            <View style={styles.tutorialContainer}>
                <Tutorial
                    ...
                    inMuseumMode={
                        this.props.route.params.project.in_museum_mode
                    }
        ...
        const question = (
            <View
                style={colorModes.contentBackgroundColorFor(
                    this.props.route.params.project.in_museum_mode,
                )}>
                <Question
        ...
        const seenThisSession =
            R.indexOf(subject.id, subjectsSeenThisSession) >= 0;
        const classificationPanel = (
            <View
                style={[
                    styles.classificationPanel,
                    colorModes.framingBackgroundColorFor(
                        this.props.route.params.project.in_museum_mode,
                    ),
                ]}>
                <ClassificationPanel
                    ...
                    {isQuestionVisible && <View>{question}</View>}
                </ClassificationPanel>
                {isQuestionVisible ? (
                    <ScrollView
                        ...
                        <View
                            style={[
                                styles.classifierContainer,
                                colorModes.contentBackgroundColorFor(
                        ...
                        {task.help !== null && (
                            <NeedHelpButton
                                ...
                                onPress={() =>
                                    this.classifierContainer.displayHelpModal()
                                }
                            />
                        )}
                        {R.length(guide.items) > 0 && (
                            <GuideButton
                                ...
                                onPress={() =>
                                    this.classifierContainer.displayFieldGuide()
                                }
...
const mapStateToProps = (state, ownProps) => {
    return {
        ...
        annotations:
            state.classifier.annotations[ownProps.route.params.workflow.id] ||
            {},
        guide: state.classifier.guide[ownProps.route.params.workflow.id] || {},
        tutorial:
            state.classifier.tutorial[ownProps.route.params.workflow.id] || {},
        needsTutorial:
            state.classifier.needsTutorial[ownProps.route.params.workflow.id] ||
            false,
        subject: state.classifier.subject || {},
        subjectsSeenThisSession:
            state.classifier.seenThisSession[
                ownProps.route.params.workflow.id
            ] || [],
    };
};
