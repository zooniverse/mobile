        ...
        .then(userResourse => {
          userResourse
            ...
            .then(preferenceCount => {
              return userResourse.get('project_preferences', {
                page_size: preferenceCount,
                sort: '-updated_at',
              });
            })
            .then(projectPreferences => {
              const sortedPreferences = sortPreferences(projectPreferences);
              const projectIDs = map(pref => {
                return pref.links.project;
              }, sortedPreferences);
              ...
              const completedTutorials =
                getCompletedTutorials(sortedPreferences);

              return apiClient
                .type('projects')
                .get({id: projectIDs, page_size: sortedPreferences.length})
                .catch(() => {
                  return null;
                })
                .then(projects => {
...
export function calculateTotalClassifications() {
  return (dispatch, getState) => {
    ...
    const totalClassifications = reduce(
      add,
      0,
      map(getCounts, keys(getState().user.projects)),
    );
    dispatch({
...
function getCompletedTutorials(projectPreferences) {
  const preferencesWithTutorials = filter(pref => {
    return !isNil(pref.preferences.tutorials_completed_at);
  }, projectPreferences);
  const extractPreference = pref => {
    return [pref.links.project, pref.preferences.tutorials_completed_at];
  };
...
function classificationCounts(projectPreferences) {
  return reduce(
    (counts, projectPreference) => {
      counts[projectPreference.links.project] =
        projectPreference.activity_count;
      return counts;
    },
    {},
    projectPreferences,
